...
FROM (
    VALUES
        ('Hiệu', 'Nguyễn', 'nguyen.hieu@vetclinic.com', 'admin'),
        ('Mây',  'Đỗ',     'do.may@vetclinic.com',     'admin'),
        ('Thư',  'Phạm',   'pham.thu@vetclinic.com',   'admin'),
        ('Như',  'Nguyễn', 'nguyen.nhu@vetclinic.com', 'admin')
) AS admin_users(first_name, last_name, email, user_type)
...
-- =========================
INSERT INTO pet (name, breed, age, user_id)
SELECT
...
JOIN PET p ON p.user_id = a.user_id
WHERE NOT EXISTS (
    SELECT 1 FROM PATIENT_REPORT pr WHERE pr.slot_id = s.slot_id
);

-- 2. Seed dữ liệu dịch vụ
INSERT INTO service (name, price, service_category) VALUES
-- Khám sức khỏe
