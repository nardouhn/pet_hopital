      ...
      if (result) {
        ...
        try {
          ...
          const updatedAuth = {
            ...
            user: {
              name: `${user?.first_name || user?.firstName || ''} ${user?.last_name || user?.lastName || ''}`.trim() || authObj?.user?.name,
              email: user?.email || authObj?.user?.email,
              pets_count: user?.pets_count ?? (user?.pets ? user.pets.length : 0),
  ...
  const handleUpdate = async (e) => {
    ...
    if (formData.newPassword) {
      ...
      if (!formData.currentPassword) {
        errors.currentPassword = 'Mật khẩu hiện tại là bắt buộc để đổi mật khẩu';
      }
    ...
    try {
      ...
      await updateProfile({ first_name: formData.firstName, last_name: formData.lastName, email: formData.email });
...
const submitAddPet = async (e) => {
  ...
  try {
    ...
    const result = await createPet({ name: petForm.name, breed: petForm.breed, age: petForm.age || null });

    // Optimistically update UI using returned pet if available
    const createdPet = result?.data?.pet || result?.pet || result?.data || null;
    if (createdPet) {
      setUserData((prev) => {
        ...
        <div className="bg-white rounded-xl shadow-sm p-8 mb-6 border border-gray-50">
          ...
          {!isEditing ? (
            <div className="space-y-4">
              <div>
                <label className="text-xs text-gray-400 block mb-1">Họ tên</label>
              ...
              <div>
                <label className="text-xs text-gray-400 block mb-2">User Type</label>
                <span className="bg-[#e0f7f7] text-[#2e94a5] px-3 py-1 rounded-full text-xs font-semibold flex items-center w-fit gap-1">
                  <CheckCircle size={12} /> {userData?.user_type || 'Customer'}
                </span>
              </div>
              <button 
            ...
            <form onSubmit={handleUpdate} className="grid grid-cols-2 gap-6">
              <div className="col-span-1">
                <label className="text-xs text-gray-400 block mb-1 uppercase">Họ</label>
                <input 
              ...
              <div className="col-span-1">
                <label className="text-xs text-gray-400 block mb-1 uppercase">Tên</label>
                <input 
                  className="w-full border border-gray-200 rounded-md p-2 focus:outline-none focus:ring-1 focus:ring-[#2e94a5]"
              ...
              <div className="col-span-2">
                <label className="text-xs text-gray-400 block mb-1 uppercase">Email</label>
                <input 
                  type="email"
                  className="w-full border border-gray-200 rounded-md p-2 focus:outline-none focus:ring-1 focus:ring-[#2e94a5]"
              ...
              <div className="col-span-1">
                <label className="text-xs text-gray-400 block mb-1 uppercase">Mật khẩu mới</label>
                <input 
                  type="password"
                  className="w-full border border-gray-200 rounded-md p-2 focus:outline-none focus:ring-1 focus:ring-[#2e94a5]"
              ...
              <div className="col-span-1">
                <label className="text-xs text-gray-400 block mb-1 uppercase">Nhập lại mật khẩu</label>
                <input 
                  type="password"
                  className="w-full border border-gray-200 rounded-md p-2 focus:outline-none focus:ring-1 focus:ring-[#2e94a5]"
              ...
              {formData.newPassword && (
                <div className="col-span-2">
                   <label className="text-xs text-red-400 block mb-1 uppercase">Mật khẩu hiện tại (để xác nhận)</label>
                   <input 
                    type="password"
                  ...
                  />
                  {formErrors.currentPassword && <p className="text-sm text-red-600 mt-1">{formErrors.currentPassword}</p>}
                </div>
              )}

              {formErrors.general && <div className="col-span-2"><p className="text-sm text-red-600">{formErrors.general}</p></div>}
              {successMessage && <div className="col-span-2"><p className="text-sm text-green-600">{successMessage}</p></div>}

              <div className="col-span-2 flex gap-3">
                <button 
                  type="submit"
                  disabled={submitting}
                  className={`flex items-center gap-2 ${submitting ? 'opacity-60 cursor-not-allowed' : ''} bg-[#2e94a5] text-white px-6 py-2 rounded-lg text-sm hover:bg-[#257a8a]`}>
                  <Edit2 size={14} /> {submitting ? 'Đang cập nhật...' : 'Xác nhận sửa'}
                </button>
                <button 
                  type="button"
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">{userData?.pets_count ?? 0} pet registered</p>
              </div>
            </div>
            <button onClick={() => setShowAddPet(true)} className="flex items-center gap-2 bg-[#2e94a5] text-white px-4 py-2 rounded-lg text-sm hover:bg-[#257a8a]">
            ...
            <div className="bg-white rounded-xl p-6 w-full max-w-md">
              ...
              <form onSubmit={submitAddPet} className="space-y-4">
                <div>
                  ...
                  <input value={petForm.name} onChange={(e) => setPetField('name', e.target.value)} className="w-full border rounded-md px-3 py-2" required />
                </div>
                <div>
                  <label className="text-sm text-gray-600 block mb-1">Breed</label>
                  <input value={petForm.breed} onChange={(e) => setPetField('breed', e.target.value)} className="w-full border rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="text-sm text-gray-600 block mb-1">Age</label>
                  <input value={petForm.age} onChange={(e) => setPetField('age', e.target.value)} className="w-full border rounded-md px-3 py-2" />
                </div>

                {petError && <p className="text-sm text-red-600">{petError}</p>}

                <div className="flex gap-3 justify-end">
                  <button type="button" onClick={() => setShowAddPet(false)} className="px-4 py-2 rounded-md border">Cancel</button>
                  <button disabled={petLoading} type="submit" className="px-4 py-2 rounded-md bg-teal-600 text-white">{petLoading ? 'Adding...' : 'Add Pet'}</button>
                </div>
              </form>
            </div>
          </div>
